...
static void loadScene(GLFWwindow* window, const int width, const int height)
{
    ...
    for (unsigned int i = 0; i < NR_LIGHTS; i++) {
        ...
        light_colors.push_back(glm::vec4(r_color, g_color, b_color, 20.0f));
    }

    camera->setCamSpeed(25.0f);

    shader_blur = new Shader("glsl/ex_46/blur.vs", "glsl/ex_46/blur.fs");
    shader_bloom = new Shader("glsl/ex_46/bloom.vs", "glsl/ex_46/bloom.fs");

    shader_g_buffer = new Shader("glsl/ex_46/g_buffer.vs", "glsl/ex_46/g_buffer.fs");
    shader_debug = new Shader("glsl/ex_46/debug_depth.vs", "glsl/ex_46/debug_depth.fs");
    shader_object = new Shader("glsl/ex_46/model_loading.vs", "glsl/ex_46/model_loading.fs");
    shader_deferred = new Shader("glsl/ex_46/deferred_shading.vs", "glsl/ex_46/deferred_shading.fs");
    shader_light = new Shader("glsl/ex_46/light.vs", "glsl/ex_46/light.fs");
    ...
    /// Global lights
    {
        for(auto *shader : config_shader) {
            ...
            for(unsigned i = 0; i < light_positions.size(); ++i) {
                ...
                float linear    = 0.7;
                float quadratic = 1.8;
                float light_max  = std::fmaxf(std::fmaxf(light_colors[i].r, light_colors[i].g), light_colors[i].b);
                float radius =
                  (-linear +  glm::sqrt(linear * linear - 4 * quadratic * (constant - (256.0 / 5.0) * light_max)))
                  / (2 * quadratic);

                shader->setFloat(s_lights + "linear",  linear);
                shader->setFloat(s_lights + "quadratic", quadratic);
                shader->setFloat(s_lights + "radius", radius);
...
int run_046(const int width, const int height)
{
